...
#  initialisation 
image_ft = fftshift(fft2(np.mean(gold.coherent_imgs['streak'], axis = 0)))
...
full_arr = np.zeros((N,gold.coherent_imgs['streak'].shape[1], gold.coherent_imgs['streak'].shape[2]), dtype=complex)

print(f"Total number of coherent images: {gold.coherent_imgs['streak'].shape}")

for i, offset in enumerate(tqdm(offsets, total = N)):
