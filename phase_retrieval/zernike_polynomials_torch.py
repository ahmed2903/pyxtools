...
class SquarePolynomialsTorch:
    def __init__(self, pupil_shape, device="cuda", dtype=torch.float32):
        self.device = device
        self.dtype = dtype

        H, W = pupil_shape
        side_x = torch.linspace(-1/torch.sqrt(torch.tensor(2.0, device=self.device, dtype=self.dtype)),
                                1/torch.sqrt(torch.tensor(2.0, device=self.device, dtype=self.dtype)),
                                W, device=self.device, dtype=self.dtype)
        side_y = torch.linspace(-1/torch.sqrt(torch.tensor(2.0, device=self.device, dtype=self.dtype)),
                                1/torch.sqrt(torch.tensor(2.0, device=self.device, dtype=self.dtype)),
                                H, device=self.device, dtype=self.dtype)
        self.X, self.Y = torch.meshgrid(side_x, side_y, indexing="xy")

        self.rho2 = self.X**2 + self.Y**2
        
    def evaluate_all(self, coeffs):
        """
        Build wavefront from coefficients.
        coeffs: (29,) tensor
        """
        X, Y, rho2 = self.X, self.Y, self.rho2
        c = coeffs

        wavefront = (
            c[0] * torch.ones_like(X) +
            c[1] * torch.sqrt(torch.tensor(6.0, device=self.device, dtype=self.dtype)) * X +
            c[2] * torch.sqrt(torch.tensor(6.0, device=self.device, dtype=self.dtype)) * Y +
            c[3] * torch.sqrt(torch.tensor(5/2.0, device=self.device, dtype=self.dtype)) * (3*rho2 - 1) +
            c[4] * 6 * X * Y +
            c[5] * 3 * torch.sqrt(torch.tensor(5/2.0, device=self.device, dtype=self.dtype)) * (X**2 - Y**2) +
            c[6] * torch.sqrt(torch.tensor(21/31.0, device=self.device, dtype=self.dtype)) * (15 * rho2 - 7) * Y +
            c[7] * torch.sqrt(torch.tensor(21/31.0, device=self.device, dtype=self.dtype)) * (15 * rho2 - 7) * X +
            c[8] * torch.sqrt(torch.tensor(5/31.0, device=self.device, dtype=self.dtype)) * (27 * X**2 - 35 * Y**2 + 6) * Y +
            c[9] * torch.sqrt(torch.tensor(5/31.0, device=self.device, dtype=self.dtype)) * (35 * X**2 - 27 * Y**2 - 6) * X +
            c[10] * (1/(2*torch.sqrt(torch.tensor(67.0, device=self.device, dtype=self.dtype)))) * (315 * rho2**2 - 240*rho2 + 31) +
            c[11] * (15/(2*torch.sqrt(torch.tensor(2.0, device=self.device, dtype=self.dtype)))) * (X**2 - Y**2) * (7*rho2 - 3) +
            c[12] * torch.sqrt(torch.tensor(42.0, device=self.device, dtype=self.dtype)) * (5 * rho2 - 3) * X * Y +
            c[13] * (3/(4*torch.sqrt(torch.tensor(134.0, device=self.device, dtype=self.dtype)))) * (10*(49*X**4 - 36*X**2 * Y**2 + 49 * Y**4) - 150*rho2 + 11) +
            c[14] * 5 * torch.sqrt(torch.tensor(42.0, device=self.device, dtype=self.dtype)) * (X**2 - Y**2) * X * Y +
            c[15] * torch.sqrt(torch.tensor(55/1966.0, device=self.device, dtype=self.dtype)) * (315*rho2**2 - 280*X**2 - 324*Y**2 + 57) * X +
            c[16] * torch.sqrt(torch.tensor(55/1966.0, device=self.device, dtype=self.dtype)) * (315*rho2**2 - 324*X**2 - 280*Y**2 + 57) * Y +
            c[17] * 0.5 * torch.sqrt(torch.tensor(3/844397.0, device=self.device, dtype=self.dtype)) * (105 * (1023*X**4 + 80*X**2 * Y**2 - 943*Y**4) - 61075 * X**2 + 39915*Y**2 + 4692) * X +
            c[18] * 0.5 * torch.sqrt(torch.tensor(3/844397.0, device=self.device, dtype=self.dtype)) * (105 * (943*X**4 - 80*X**2 * Y**2 - 1023*Y**4) - 39915 * X**2 + 61075*Y**2 + 4692) * Y +
            c[19] * 0.25 * torch.sqrt(torch.tensor(7/859.0, device=self.device, dtype=self.dtype)) * (6 * (693*X**4 - 500*X**2 * Y**2 + 525*Y**4) - 1810 * X**2 - 450*Y**2 + 165) * X +
            c[20] * 0.25 * torch.sqrt(torch.tensor(7/859.0, device=self.device, dtype=self.dtype)) * (6 * (525*X**4 - 500*X**2 * Y**2 + 693*Y**4) - 450 * X**2 - 1810*Y**2 + 165) * Y +
            c[21] * 0.25 * torch.sqrt(torch.tensor(65/849.0, device=self.device, dtype=self.dtype)) * (1155 * rho2**3 - 15 * (91 * X**4 + 198 * X**2 * Y**2 + 91*Y**4) + 453*rho2 - 31) +
            c[22] * torch.sqrt(torch.tensor(33/3923.0, device=self.device, dtype=self.dtype)) * (1575 * rho2**2 - 1820*rho2 + 471) * X * Y +
            c[23] * (24/4) * torch.sqrt(torch.tensor(65/1349.0, device=self.device, dtype=self.dtype)) * (165 * rho2**2 - 140*rho2 + 27) * (X**2 - Y**2) +
            c[24] * 7 * torch.sqrt(torch.tensor(33/2.0, device=self.device, dtype=self.dtype)) * (9 * rho2 - 5) * X * Y * (X**2 - Y**2) +
            c[25] * (1/(8*torch.sqrt(torch.tensor(849.0, device=self.device, dtype=self.dtype)))) * (42 * (1573 * X**6 - 375 * X**4 * Y**2 - 375 * X**2 * Y**4 + 1573 * Y**6) - 60*(707*X**4 - 225 * X**2 * Y**2 + 707 * Y**4) + 6045*rho2 - 245) +
            c[26] * (1/(2*torch.sqrt(torch.tensor(7846.0, device=self.device, dtype=self.dtype)))) * (14 * (2673 * X**4 - 2500 * X**2 * Y**2 + 2673 * Y**4) - 10290*rho2 + 1305) * X * Y +
            c[27] * (21/(8*torch.sqrt(torch.tensor(1349.0, device=self.device, dtype=self.dtype)))) * (3146 * X**6 - 2250 * X**4 * Y**2 + 2250 *X**2 * Y**4 - 3146*Y**6 - 1770 *(X**4 - Y**4) + 245*(X**2-Y**2)) +
            c[28] * (-13.79189793 + 150.92209099*X**2 + 117.01812058*Y**2 - 352.15154565*X**4 - 657.27245247 *X**2 * Y**2 - 291.12439892*Y**4 + 222.62454035*X**6 + 667.87362106 * X**4 * Y**2 +667.87362106 *X**2 * Y**4 + 222.62454035* Y**6)*Y
        )

        return wavefront

    def get_zernike_wavefront_torch(self, coefficients):
        """
        coefficients: torch.Tensor of shape (29,)
        basis_tensor: torch.Tensor of shape (29, H, W)
        """
        
        coeffs = coefficients.view(-1, 1, 1)  # (29,1,1)
        wavefront = torch.sum(coeffs * self.basis_tensor, dim=0)  # (H, W)
