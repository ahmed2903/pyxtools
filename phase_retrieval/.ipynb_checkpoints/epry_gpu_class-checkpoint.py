...
class EPRy:
    ...
    def prepare(self, **kwargs):
        ...
        self.kout_vec = cp.array(self.kout_vec)
    ...
    def _prep_images(self):
        ...
        self.images = cp.abs(cp.array(self.images))
    ...
    def _load_pupil(self):
        ...
        dims = cp.round((self.kx_max_n - self.kx_min_n)/self.dkx), cp.round((self.ky_max_n - self.ky_min_n)/self.dky)
        dims = make_dims_even(dims)
        dims = (int(dims[0]), int(dims[1]))
        full_array = cp.zeros(dims)
        
        if isinstance(self.pupil_func, str):
            phase = cp.load(self.pupil_func)
        elif isinstance(self.pupil_func, cp.ndarray):
        ...
        self.pupil_func = cp.exp(1j*full_array)
    ...
    def _initiate_recons_images(self):
        ...
        if self.hr_obj_image is None and self.hr_fourier_image is None:
            ...
            self.hr_obj_image = cp.asarray(self.hr_obj_image).copy()
            self.hr_fourier_image = fftshift(fft2(self.hr_obj_image))
            
        elif self.hr_fourier_image is not None:
            self.hr_fourier_image = cp.asarray(self.hr_fourier_image).copy()
    ...
    def iterate(self, iterations:int, live_plot=False):
        ...
        for it in range(iterations):
            ...
            self.losses.append(self.iter_loss.get()/self.num_images.get())
            self.iters_passed += 1
    ...
    def _post_process(self):
        ...
        self.hr_obj_image = cp.asnumpy(self.hr_obj_image)
        self.hr_fourier_image = cp.asnumpy(self.hr_fourier_image)
        self.pupil_func = cp.asnumpy(self.pupil_func)
        
    def compute_weight_fac(self, func):
        """Compute weighting factor for phase retrieval update."""
        
        mod = cp.abs(func) ** 2
        return cp.conjugate(func) / (mod.max() + 1e-23)

    def _compute_loss(self, pred, target):
        
        return cp.sqrt(cp.sum(cp.abs(pred - target) ** 2)) 
    
    def _update_spectrum(self, image, kx_iter, ky_iter):
        
        """Handles the Fourier domain update."""
        kx_cidx = cp.round((kx_iter - self.kx_min_n) / self.dkx)
        kx_lidx = cp.round(max(kx_cidx - self.omega_obj_x / (2 * self.dkx), 0))
        kx_hidx = cp.round(kx_cidx + self.omega_obj_x / (2 * self.dkx)) + (1 if self.nx_lr % 2 != 0 else 0)
        
        ky_cidx = cp.round((ky_iter - self.ky_min_n) / self.dky)
        ky_lidx = cp.round(max(ky_cidx - self.omega_obj_y / (2 * self.dky), 0))
        ky_hidx = cp.round(ky_cidx + self.omega_obj_y / (2 * self.dky)) + (1 if self.ny_lr % 2 != 0 else 0)
        ...
        image_lr_update = cp.sqrt(image) * cp.exp(1j * cp.angle(image_lr))
        ...
        if cp.any(cp.isnan(cp.sqrt(image))):
            raise ValueError(f"There is a Nan in the image")
        if cp.any(cp.isnan(cp.angle(image_lr))):
            raise ValueError(f"There is a Nan in the image_lr")
        if cp.any(cp.isnan(image_lr_update)):
        ...
        image_lr_new = cp.abs(ifft2(ifftshift(self.hr_fourier_image[kx_lidx:kx_hidx, ky_lidx:ky_hidx]*self.pupil_func[kx_lidx:kx_hidx, ky_lidx:ky_hidx])))**2
        
        self.iter_loss+=self._compute_loss(image_lr_new, image).get()
    ...
    def _initialize_live_plot(self):
        ...
        object_np= cp.asnumpy(self.hr_obj_image)
        fourier_np = cp.asnumpy(self.hr_fourier_image)
        
        # Initialize empty images
        fig, axes = plt.subplots(2, 2, figsize=(8, 8))
                
        # Initialize the plots with the initial image
        img_amp = axes[0,0].imshow(np.abs(object_np), vmin =.2, vmax = 1, cmap='viridis')
        axes[0,0].set_title("Object Amplitude")
        cbar_amp = plt.colorbar(img_amp, ax=axes[0,0])

        img_phase = axes[0,1].imshow(np.angle(object_np), cmap='viridis')
        axes[0,1].set_title("Object Phase")
        img_phase.set_clim(-cp.pi, np.pi)
        cbar_phase = plt.colorbar(img_phase, ax=axes[0,1])
        
        
        fourier_amp = axes[1,0].imshow(np.abs(fourier_np), cmap='viridis')
    ...
    def _update_live_plot(self, img_amp, img_phase, fourier_amp, loss_im, fig, it, axes):
        ...
        """
        amplitude_obj = cp.asnumpy(cp.abs(self.hr_obj_image))
        phase_obj = cp.asnumpy(cp.angle(self.hr_obj_image))
        amplitude_ft = cp.asnumpy(cp.abs(self.hr_fourier_image))
    ...
    def save_reconsturction(self, file_path):
        ...
        with h5py.File(file_path, "w") as h5f:
            ...
            amp = cp.abs(self.hr_obj_image)
            pha = cp.angle(self.hr_obj_image)
            ...
            # Save spectrum 
            amp = cp.abs(self.hr_fourier_image)
            pha = cp.angle(self.hr_fourier_image)
            ...
            amp = cp.abs(self.pupil_func)
            pha = cp.angle(self.pupil_func)
            ...
            losses.create_dataset("main_loss_values", data=cp.array(self.losses), compression="gzip")
...
class EPRy_lr(EPRy):
    ...
    def _initiate_recons_images(self):
        if self.hr_obj_image is None and self.hr_fourier_image is None: 
            self.hr_obj_image = cp.ones_like(self.images[0]).astype(complex)
            self.hr_fourier_image = cp.ones_like(self.images[0]).astype(complex)
    ...
    def _update_spectrum(self, image, kx_iter, ky_iter):
        ...
        image_lr_new = cp.abs(ifft2(ifftshift(self.hr_fourier_image*self.pupil_func[kx_lidx:kx_hidx, ky_lidx:ky_hidx])))**2
...
class EPRy_fsc_lr(EPRy_lr):
    ...
    def _center_fourier_spectrum(self):
        ...
        total_power = cp.sum(power) + 1e-8
        cx = cp.sum(X * power) / total_power
        cy = cp.sum(Y * power) / total_power

        # Desired center
        cx_target = nx // 2
        cy_target = ny // 2

        shift_x = -cp.round(cx_target - cx)
        shift_y = -cp.round(cy_target - cy)
...
class EPRy_upsample(EPRy_lr):
    ...
    def _prep_images(self):
        ...
        self.images = cp.array(self.images)
...
class EPRy_pad(EPRy_lr):        
    ...
    def _prep_images(self):
        ...
        self.images = pad_to_double_batch(self.images)
        self.images = cp.array(self.images)
