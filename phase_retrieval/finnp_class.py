...
from .schedulers import ShrinkWrap, WeightScheduler
...
class FINN:
    ...
    def set_secondary_loss_func(self, loss_func, delta, amplitude_based = True, **kwargs):
        """
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments for the loss function.
        """
        self.amplitude_based = amplitude_based
        func_args = self.GetKwArgs(loss_func, kwargs)
        if isinstance(loss_func, type):  
            self.loss_fn = loss_func(**func_args)  # Instantiating the loss function
        else:
    ...
    def iterate(self, epochs, optim_flag = 5, live_flag = None, n_jobs = -1):
        ...
        if live_flag is not None:
            fig, ax, line_mse = self._init_live_plot()
        ...
        for epoch in tqdm(range(epochs), desc="Processing", total=epochs, unit="Epochs"):
            ...
            # Update loss list
            self.main_losses.append(self.epoch_loss.cpu().detach().numpy())
            ...
            if live_flag is not None and epoch % live_flag == 0:
                ...
                self._update_live_loss(fig,ax,line_mse)
    ...
    def _process_image(self, image, kx_iter, ky_iter):        
        ...
        if self.sec_loss_fn is not None:
            ...
            self.epoch_loss += self.delta * sec_loss
    ...
    def _init_live_plot(self):
        ...
        return fig, ax, line_mse
        
    def _update_live_loss(self, fig, ax,line_mse):
        """
        ...
        ax.set_title(f"Epoch = {self.epochs_passed-1}, MSE Loss = {self.epoch_loss:.2f}")
        ax.set_yscale('log')
        ax.relim()
        ax.autoscale_view()
...
class FINN_alphatv(FINN):
    ...
    def iterate(self, epochs, optim_flag = 5, live_flag = None, n_jobs = -1):
        ...
        for epoch in tqdm(range(epochs), desc="Processing", total=epochs, unit="Epochs"):
            ...
            self.epoch_loss = self.mse_loss * self.alpha * self.tv_loss
            ...
            # Updating live plot
            if live_flag is not None and epoch % live_flag == 0:
                self._update_live_loss(fig,ax,line_mse,line_tv)
    ...
    def set_tv_param(self, start_epoch, increase_epochs, alpha_max, plat_epochs, gamma, min_alpha):
        ...
        """
        self.tv_scheduler = WeightScheduler(
    ...
    def _init_live_plot(self):
        ...
        return fig, ax, line_mse, line_tv
        
    def _update_live_loss(self, fig, ax,line_mse,line_tv):
        """
        Update the live loss plot during training.
...
class FINN_support(FINN):
    ...
    def iterate(self, epochs, optim_flag = 5, live_flag = None, n_jobs = -1):
        ...
        if live_flag is not None:
            fig, ax, line_mse, line_supp = self._init_live_plot()
        ...
        for epoch in tqdm(range(epochs), desc="Processing", total=epochs, unit="Epochs"):
            ...
            # Compute gradients
            self.alpha = self.sw_scheduler.get_alpha(self.epochs_passed)
            self.epoch_loss = self.mse_loss + self.supp_loss * self.alpha
            ...
            # Updating live plot
            if live_flag is not None and epoch % live_flag == 0:
                self._update_live_loss(fig,ax,line_mse,line_supp)
    ...
    def set_sw_weights(self, start_epoch, increase_epochs, alpha_max, plat_epochs, gamma, min_alpha):
        """
        Initialize the alpha tv scheduler.
        ...
        """
        self.sw_scheduler = WeightScheduler(
                    start_epoch=start_epoch,
                    increase_epochs=increase_epochs,
                    alpha_max=alpha_max,
                    plateau_epochs=plat_epochs,    
    ...
    def _init_live_plot(self):
        ...
        return fig, ax, line_mse, line_supp
        
    def _update_live_loss(self, fig, ax,line_mse,line_supp):
        """
        Update the live loss plot during training.
    ...
    def save_reconsturction(self, file_path):
        ...
        alpha_meta = {
        "Start Epoch" : self.sw_scheduler.start_epoch,
        "Alpha Max": self.sw_scheduler.alpha_max,
        "Increase Epochs": self.sw_scheduler.increase_epochs,
        "Plateau Epochs": self.sw_scheduler.plateau_epochs,
        "Gamma": self.sw_scheduler.gamma,
        "Alpha Min": self.sw_scheduler.min_alpha
        }
        ...
        with h5py.File(file_path, "w") as h5f:
            ...
            kdata = h5f.create_group("K-space Params")
            supp_weights = h5f.create_group("Support Weight")
